	...
	for(i = 0; i < NUM_WYCHEPROOF_XDH_TESTS; i++){
		...
		if(t->xdh_alg == X25519){
			...
			if(t->ourpubkeylen != 0){
				...
				ret = are_equal(t->ourpubkey, pubkey_check, alglen, &check); EG(ret, err);
				if(!check){
			...
			ret = are_equal(t->sharedsecret, sharedsecret_check, alglen, &check); EG(ret, err);
			if(!check){
...
static int check_wycheproof_ecdh(void)
{
	...
	for(i = 0; i < NUM_WYCHEPROOF_ECDH_TESTS; i++){
		...
		ret = local_memset(&peerpub_key, 0, sizeof(peerpub_key)); EG(ret, err);
		ret = local_memset(&ourpub_key, 0, sizeof(ourpub_key)); EG(ret, err);
		ret = local_memset(&ourpub_key_check, 0, sizeof(ourpub_key_check)); EG(ret, err);
		ret = local_memset(&priv_key, 0, sizeof(priv_key)); EG(ret, err);
		ret = local_memset(&params, 0, sizeof(params)); EG(ret, err);
		ret = local_memset(sharedsecret_check, 0, sizeof(sharedsecret_check)); EG(ret, err);
		...
		MUST_HAVE((sharedsecretsize < sizeof(sharedsecret_check)), ret, err);
		...
		MUST_HAVE((serialized_pub_key_size <= sizeof(serialized_pub_key)), ret, err);
		MUST_HAVE((serialized_pub_key_size <= sizeof(serialized_pub_key_check)), ret, err);
		...
		if(t->ourpubkeylen != 0){
			...
			ret = are_equal(serialized_pub_key, serialized_pub_key_check, serialized_pub_key_size, &check); EG(ret, err);
			if(!check){
		...
		if(t->compressed > 0){
			...
			ret = local_memcpy(serialized_pub_key, t->peerpubkey, serialized_pub_key_size); EG(ret, err);
		}
		...
		/* Compare */
		ret = are_equal(sharedsecret_check, t->sharedsecret, sharedsecretsize, &check); EG(ret, err);
		if(!check){
...
static int check_wycheproof_hmac(void)
{
	...
	for(i = 0; i < NUM_WYCHEPROOF_HMAC_TESTS; i++){
		...
		ret = local_memset(&hmac_res, 0, sizeof(hmac_res)); EG(ret, err);
		...
		/* Compare */
		ret = are_equal(hmac_res, t->tag, t->taglen, &check); EG(ret, err);
		if((!check) && (t->result >= 0)){
